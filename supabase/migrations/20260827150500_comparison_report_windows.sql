-- WP-47A: the UI's default comparison needs two adjacent 30-day windows.
--
-- Reporting v3 accepts at most a 31-day date difference, so one schedule cannot
-- load all 60 days. Keep the existing restatement block and add an equally sized
-- comparison block immediately before it. Report windows end yesterday: the
-- profile-local current day is incomplete and must never enter a comparison.
...
-- Existing profiles already have their defaults, so the TypeScript provisioner
-- will not visit them. Seed exactly the new variant from each existing
-- restatement schedule once; an operator who removes it later stays in control.
