...
import { createSpWriteRuntimeLedger, type LoadVerifiedSpWriteExecutionIdentity } from './sp-write-persistence.js';
...
export async function loadSpWriteOperationDetail(
  handle: Pick<DbHandle, 'sql'>, identity: LoadVerifiedSpWriteExecutionIdentity,
): Promise<SpWriteOperationDetail> {
  const evidence = await createSpWriteRuntimeLedger(handle).loadVerifiedExecution(identity);
