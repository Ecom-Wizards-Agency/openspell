-- WP-197 offline evidence for the exact 45-file hosted prefix.
...
checks(check_key, expected, observed) as (
  select 'prefix.file_count', '45', file_count::text from ledger_state
  union all
  select 'prefix.ledger_sha256', 'ac7e960282c6f7d999656eb0f0a87ca84deef772b5c9fa974cd75b7893a44a5b',
         case when mismatch_count = 0 and file_count = 45
              then 'ac7e960282c6f7d999656eb0f0a87ca84deef772b5c9fa974cd75b7893a44a5b' else repeat('0', 64) end
  from ledger_state
  union all
  select 'prefix.terminal_version', '20260901050000', terminal_version from ledger_state
  union all
  select 'prefix.version_set_mismatch', '0', mismatch_count::text from ledger_state
  union all
  select 'catalog.relevant_item_count', '2731', item_count::text from catalog_snapshot
  union all
  select 'catalog.relevant_sha256',
         '0ae246cd50590e5951b206b2540afe7b945eba8fde8d56f7be6df2f963c89b39',
