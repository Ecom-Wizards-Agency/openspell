-- WP-197 offline evidence for the exact 44-file hosted prefix.
...
checks(check_key, expected, observed) as (
  select 'prefix.file_count', '44', file_count::text from ledger_state
  union all
  select 'prefix.ledger_sha256', '0e408d86b6fad5713e459b4369bbf5a6c39a3174c4ea6845bfb1ca262de647b1',
         case when mismatch_count = 0 and file_count = 44
              then '0e408d86b6fad5713e459b4369bbf5a6c39a3174c4ea6845bfb1ca262de647b1' else repeat('0', 64) end
  from ledger_state
  union all
  select 'prefix.terminal_version', '20260901040000', terminal_version from ledger_state
  union all
  select 'prefix.version_set_mismatch', '0', mismatch_count::text from ledger_state
  union all
  select 'catalog.relevant_item_count', '2633', item_count::text from catalog_snapshot
  union all
  select 'catalog.relevant_sha256',
         '4f7ea6d9473e84a002f863097982ed6e0baa6c59acd3806115c16df94d80ecbf',
  ...
  join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
  where namespace.nspname in ('app', 'public')
    and relation.relname = any(array['recommendation_preview_batches', 'recommendation_run_campaigns', 'recommendation_claim_authority']::text[])
  union all
