-- WP-197 offline evidence for the exact 43-file hosted prefix.
...
checks(check_key, expected, observed) as (
  select 'prefix.file_count', '43', file_count::text from ledger_state
  union all
  select 'prefix.ledger_sha256', 'd1359b1fd9dfa5b1ed8c6669df8323a35b4684fbbb3d7a0fc739488aee1d9530',
         case when mismatch_count = 0 and file_count = 43
              then 'd1359b1fd9dfa5b1ed8c6669df8323a35b4684fbbb3d7a0fc739488aee1d9530' else repeat('0', 64) end
  from ledger_state
  union all
  select 'prefix.terminal_version', '20260901030000', terminal_version from ledger_state
  union all
  select 'prefix.version_set_mismatch', '0', mismatch_count::text from ledger_state
  union all
  select 'catalog.relevant_item_count', '2579', item_count::text from catalog_snapshot
  union all
  select 'catalog.relevant_sha256',
         'de4c7259a2a0a649f4b99c11e4cb2f61b6121f1cae5b37dedb98122ede91f1b6',
  ...
  join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
  where namespace.nspname in ('app', 'public')
    and relation.relname = any(array['report_worker_claim_authority', 'recommendation_preview_batches', 'recommendation_run_campaigns', 'recommendation_claim_authority']::text[])
  union all
