-- WP-197 offline evidence for the exact 46-file hosted prefix.
...
checks(check_key, expected, observed) as (
  select 'prefix.file_count', '46', file_count::text from ledger_state
  union all
  select 'prefix.ledger_sha256', 'baef4df400ed7a045395322667e1d3ac61fa27075b2d36bb855071a6bfe20458',
         case when mismatch_count = 0 and file_count = 46
              then 'baef4df400ed7a045395322667e1d3ac61fa27075b2d36bb855071a6bfe20458' else repeat('0', 64) end
  from ledger_state
  union all
  select 'prefix.terminal_version', '20260901060000', terminal_version from ledger_state
  union all
  select 'prefix.version_set_mismatch', '0', mismatch_count::text from ledger_state

  union all
  select 'catalog.relevant_item_count', '2920', item_count::text from catalog_snapshot
  union all
  select 'catalog.relevant_sha256',
         '61c779cfb9b993575604d4ae088e83433d75d6905629860edd7ae3b8614035ea',
  ...
  select 'wp187.service_execute_set',
         'acquire_sp_write_dispatch_lease_for_claim,append_sp_write_observation,append_sp_write_provider_result,claim_sp_write_outbox,complete_sp_write_outbox_claim,defer_sp_write_outbox_claim,record_sp_write_bounded_authorization,record_sp_write_plan,renew_sp_write_outbox_claim,reserve_sp_write_provider_call_for_claim,start_sp_write_execution',
  ...
  union all
  select 'wp192.events_empty', '0',
         (select count(*) from app.sp_write_outbox_delivery_events)::text
  union all
  select 'wp192.head_count_matches_outbox',
         (select count(*) from public.sp_write_outbox)::text,
         (select count(*) from app.sp_write_outbox_delivery_heads)::text
  union all
  select 'wp192.head_genesis_mismatch', '0', count(*)::text
  from public.sp_write_outbox outbox
  full join app.sp_write_outbox_delivery_heads head
    on head.org_id = outbox.org_id
   and head.profile_id = outbox.profile_id
   and head.outbox_id = outbox.outbox_id
  where outbox.outbox_id is null or head.outbox_id is null
     or head.state <> 'available' or head.claim_epoch <> 0
     or head.transition_sequence <> 0 or head.attempt_count <> 0
     or head.claimant_id is not null or head.token_digest is not null
     or head.claimed_at is not null or head.lease_expires_at is not null
     or head.completed_at is not null
     or head.available_at is distinct from outbox.created_at
  union all
  select 'wp192.head_or_event_direct_grants', '0', count(*)::text
  from pg_catalog.pg_class relation
  join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
  cross join lateral aclexplode(relation.relacl) acl
  where namespace.nspname = 'app'
    and relation.relname in ('sp_write_outbox_delivery_heads', 'sp_write_outbox_delivery_events')
    and acl.grantee in (0, 'anon'::regrole, 'authenticated'::regrole, 'service_role'::regrole)
  union all
  select 'wp192.service_execute_set',
         'acquire_sp_write_dispatch_lease_for_claim,append_sp_write_observation,append_sp_write_provider_result,claim_sp_write_outbox,complete_sp_write_outbox_claim,defer_sp_write_outbox_claim,record_sp_write_bounded_authorization,record_sp_write_plan,renew_sp_write_outbox_claim,reserve_sp_write_provider_call_for_claim,start_sp_write_execution',
         coalesce(string_agg(distinct routine.proname, ',' order by routine.proname), '')
  from pg_catalog.pg_proc routine
  join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
  ...
  where namespace.nspname = 'app' and routine.proname like '%sp_write%'
    and acl.grantee = 'service_role'::regrole and acl.privilege_type = 'EXECUTE'
  union all
  select 'wp194.claim_tokens_nonnull', '0',
         count(*) filter (where claim_token is not null)::text
  from public.sync_jobs
  union all
  select 'wp194.report_authority', 'legacy:0:1',
         coalesce(string_agg(protocol || ':' || epoch::text, ',' order by singleton), '')
         || ':' || count(*)::text
  from app.report_worker_claim_authority
  union all
  select 'wp194.authenticated_table_select', 'false',
         has_table_privilege('authenticated', 'public.sync_jobs', 'SELECT')::text
  union all
  select 'wp194.authenticated_claim_token_select', 'false',
         has_column_privilege('authenticated', 'public.sync_jobs', 'claim_token', 'SELECT')::text
  union all
  select 'wp194.authenticated_safe_column_count', '20', count(*)::text
  from information_schema.columns column_def
  where column_def.table_schema = 'public' and column_def.table_name = 'sync_jobs'
    and column_def.column_name <> 'claim_token'
    and has_column_privilege('authenticated', 'public.sync_jobs',
                             column_def.column_name, 'SELECT')
  union all
  select 'wp194.service_fenced_execute_count', '5', count(distinct routine.proname)::text
  from pg_catalog.pg_proc routine
  join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
  cross join lateral aclexplode(routine.proacl) acl
  where namespace.nspname = 'public'
    ...
    and acl.grantee = 'service_role'::regrole and acl.privilege_type = 'EXECUTE'
  union all
  select 'wp195.preview_batches_empty', '0',
         (select count(*) from public.recommendation_preview_batches)::text
  union all
  select 'wp195.run_campaigns_empty', '0',
         (select count(*) from public.recommendation_run_campaigns)::text
  union all
  select 'wp195.historic_scope_columns_nonnull', '0', count(*)::text
  from public.recommendation_runs
  where batch_id is not null or scope_version is not null or scope_count is not null
     or scope_fingerprint is not null or strategy_goal is not null or job_id is not null
  union all
  select 'wp195.scope_relation_rls_count', '2', count(*)::text
  from pg_catalog.pg_class relation
  join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
  where namespace.nspname = 'public'
    and relation.relname in ('recommendation_preview_batches', 'recommendation_run_campaigns')
    and relation.relrowsecurity
  union all
  select 'wp195.tenant_read_policy_count', '2', count(*)::text
  from pg_catalog.pg_policy policy
  join pg_catalog.pg_class relation on relation.oid = policy.polrelid
  join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
  where namespace.nspname = 'public'
    and relation.relname in ('recommendation_preview_batches', 'recommendation_run_campaigns')
    and policy.polname = 'tenant_read'
    and policy.polcmd = 'r'
    and 'authenticated'::regrole = any(policy.polroles)
  union all
  select 'wp195.tenant_read_policy_shape_mismatch', '0', count(*)::text
  from pg_catalog.pg_policy policy
  join pg_catalog.pg_class relation on relation.oid = policy.polrelid
  join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
  where namespace.nspname = 'public'
    and relation.relname in ('recommendation_preview_batches', 'recommendation_run_campaigns')
    and policy.polname = 'tenant_read'
    and (not policy.polpermissive or policy.polcmd <> 'r'
      or policy.polwithcheck is not null
      or pg_catalog.pg_get_expr(policy.polqual, policy.polrelid) <> 'is_org_member(org_id)'
      or policy.polroles <> array['authenticated'::regrole]::oid[])
  union all
  select 'wp195.authenticated_select_count', '2', count(*)::text
  from unnest(array[
    'public.recommendation_preview_batches',
    'public.recommendation_run_campaigns'
  ]::text[]) relation_name
  where has_table_privilege('authenticated', relation_name, 'SELECT')
  union all
  select 'wp195.unsafe_api_mutation_grants', '0', count(*)::text
  from unnest(array[
    'public.recommendation_preview_batches',
    'public.recommendation_run_campaigns'
  ]::text[]) relation_name
  cross join unnest(array['anon', 'authenticated']::text[]) role_name
  where has_table_privilege(role_name, relation_name,
                            'INSERT,UPDATE,DELETE,TRUNCATE,REFERENCES,TRIGGER')
  union all
  select 'wp195.anon_select_count', '0', count(*)::text
  from unnest(array[
    'public.recommendation_preview_batches',
    'public.recommendation_run_campaigns'
  ]::text[]) relation_name
  ...
  union all
  select 'wp196.historic_execution_lineage_nonnull', '0',
         count(*) filter (where execution_lineage is not null)::text
  from public.recommendation_runs
  union all
  select 'wp196.recommendation_authority', 'legacy:legacy:0:null:1',
         coalesce(string_agg(protocol || ':' || admission || ':' || epoch::text || ':'
                            || coalesce(authorized_revision, 'null'), ','
                            order by singleton), '') || ':' || count(*)::text
  from app.recommendation_claim_authority
  union all
  select 'wp196.recommendation_authority_direct_grants', '0', count(*)::text
  from pg_catalog.pg_class relation
  join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
  cross join lateral aclexplode(relation.relacl) acl
  where namespace.nspname = 'app'
    and relation.relname = 'recommendation_claim_authority'
    and acl.grantee in (
  ...
  union all
  select 'wp196.role_attribute_mismatch', '0', count(*)::text
  from pg_catalog.pg_roles role_def
  where role_def.rolname in (
      'openspell_recommendation_worker', 'openspell_recommendation_executor'
    )
    and (role_def.rolcanlogin or role_def.rolinherit or role_def.rolsuper
      or role_def.rolcreatedb or role_def.rolcreaterole or role_def.rolreplication
      or role_def.rolbypassrls or role_def.rolvaliduntil is not null
      or role_def.rolconnlimit <> -1)
  union all
  select 'wp196.role_count', '2', count(*)::text
  from pg_catalog.pg_roles where rolname in (
    'openspell_recommendation_worker', 'openspell_recommendation_executor'
  )
  union all
  select 'wp196.unsafe_membership_edges', '0', count(*)::text
  from pg_catalog.pg_auth_members membership
  ...
  union all
  select 'wp196.executor_schema_usage', 'true:true',
         has_schema_privilege('openspell_recommendation_executor', 'public', 'USAGE')::text
         || ':' || has_schema_privilege('openspell_recommendation_executor', 'app', 'USAGE')::text
  union all
  select 'wp196.executor_schema_create', 'false:false',
         has_schema_privilege('openspell_recommendation_executor', 'public', 'CREATE')::text
         || ':' || has_schema_privilege('openspell_recommendation_executor', 'app', 'CREATE')::text
  union all
  select 'wp196.worker_schema_privileges', 'true:false:false:false',
         has_schema_privilege('openspell_recommendation_worker', 'public', 'USAGE')::text
         || ':' || has_schema_privilege('openspell_recommendation_worker', 'app', 'USAGE')::text
         || ':' || has_schema_privilege('openspell_recommendation_worker', 'public', 'CREATE')::text
         || ':' || has_schema_privilege('openspell_recommendation_worker', 'app', 'CREATE')::text
  union all
  select 'wp196.worker_direct_relation_privileges', '0', count(*)::text
  from pg_catalog.pg_class relation
  cross join lateral aclexplode(relation.relacl) acl
  where acl.grantee = 'openspell_recommendation_worker'::regrole
  union all
  select 'wp196.worker_explicit_execute_count', '11', count(*)::text
  from pg_catalog.pg_proc routine
  cross join lateral aclexplode(routine.proacl) acl
  where acl.grantee = 'openspell_recommendation_worker'::regrole
    and acl.privilege_type = 'EXECUTE'
  union all
  select 'wp196.executor_relation_acl_count', '26', count(*)::text
  from pg_catalog.pg_class relation
  cross join lateral aclexplode(relation.relacl) acl
  where acl.grantee = 'openspell_recommendation_executor'::regrole
  union all
  select 'wp196.executor_policy_count', '24', count(*)::text
  from pg_catalog.pg_policy policy
  where 'openspell_recommendation_executor'::regrole = any(policy.polroles)
  union all
  select 'wp196.executor_owned_routine_count', '9', count(*)::text
  from pg_catalog.pg_proc routine
  where routine.proowner = 'openspell_recommendation_executor'::regrole
  union all
  select 'wp196.public_execute_removed', '0', count(*)::text
  from pg_catalog.pg_proc routine
  join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
  cross join lateral aclexplode(routine.proacl) acl
  where namespace.nspname = 'public'
    and routine.proname in ('apply_cooldown_conflicts', 'tag_subtree')
    and acl.grantee = 0 and acl.privilege_type = 'EXECUTE'
  union all
  select 'wp196.worker_ambient_execute_removed', '0', count(*)::text
  from pg_catalog.pg_proc routine
  join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
  where namespace.nspname = 'public'
    and routine.proname in ('apply_cooldown_conflicts', 'tag_subtree')
    and has_function_privilege(
          'openspell_recommendation_worker', routine.oid, 'EXECUTE'
        )
  union all
  select 'wp196.recommendation_scheduler_source', 'true',
         coalesce((
           select count(*) = 1 and bool_and(
             pg_catalog.pg_get_functiondef(routine.oid)
               like '%job_type <> ''recommendations.run''%'
           )
           from pg_catalog.pg_proc routine
           join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
           where namespace.nspname = 'public'
             and routine.proname = 'enqueue_due_schedules'
             and pg_catalog.pg_get_function_identity_arguments(routine.oid)
               = 'p_now timestamp with time zone'
         ), false)::text
  union all
  select 'wp196.required_trigger_missing', '0', count(*)::text
  from (values
    ('audit_log', 'a_recommendation_authority_prelock'),
    ('audit_log', 'audit_log_human_lineage_validate'),
    ('audit_log', 'audit_log_recommendation_execution_guard'),
    ('recommendation_runs', 'a_recommendation_authority_prelock'),
    ('recommendation_runs', 'recommendation_runs_execution_guard'),
    ('recommendation_runs', 'recommendation_runs_human_lineage_validate'),
    ('recommendations', 'a_recommendation_authority_prelock'),
    ('recommendations', 'recommendations_execution_guard'),
    ('recommendations', 'recommendations_human_lineage_insert_delete_validate'),
    ('recommendations', 'recommendations_human_lineage_structure_validate'),
    ('sync_jobs', 'a_recommendation_authority_prelock'),
    ('sync_jobs', 'sync_jobs_recommendation_admission_gate'),
    ('sync_jobs', 'sync_jobs_recommendation_admission_validate')
  ) expected(relation_name, trigger_name)
  where not exists (
    select 1 from pg_catalog.pg_trigger trigger_def
    join pg_catalog.pg_class relation on relation.oid = trigger_def.tgrelid
    join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
    where namespace.nspname = 'public'
      and relation.relname = expected.relation_name
      and trigger_def.tgname = expected.trigger_name
      and not trigger_def.tgisinternal and trigger_def.tgenabled = 'O'
  )
  union all
  select 'wp196.service_control_execute_missing', '0', count(*)::text
  from (values
    ('block_recommendation_admission', 'p_expected_epoch bigint'),
    ('activate_recommendation_fenced_claims', 'p_expected_epoch bigint, p_revision text'),
    ('authorize_recommendation_scoped_admission', 'p_expected_epoch bigint, p_revision text'),
    ('rebind_recommendation_fenced_revision',
     'p_expected_epoch bigint, p_old_revision text, p_new_revision text')
  ) expected(routine_name, arguments)
  where not exists (
    select 1 from pg_catalog.pg_proc routine
    join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
    cross join lateral pg_catalog.aclexplode(routine.proacl) acl
    where namespace.nspname = 'public' and routine.proname = expected.routine_name
      and pg_catalog.pg_get_function_identity_arguments(routine.oid) = expected.arguments
      and acl.grantee = 'service_role'::regrole and acl.privilege_type = 'EXECUTE'
  )
  union all
  select 'wp196.executor_relation_acl_missing', '0', count(*)::text
  from unnest(array['public.ad_groups:SELECT', 'public.ad_profiles:SELECT', 'public.apply_batches:SELECT', 'public.apply_rows:SELECT', 'public.audit_log:INSERT', 'public.audit_log_id_seq:SELECT', 'public.audit_log_id_seq:USAGE', 'public.bid_series_daily:SELECT', 'public.campaigns:SELECT', 'public.fact_profile_daily:SELECT', 'public.fact_sb_daily:SELECT', 'public.fact_sd_daily:SELECT', 'public.fact_sp_target_daily:SELECT', 'public.keywords:SELECT', 'public.product_ads:SELECT', 'public.rank_observations:SELECT', 'public.' || 'recommendation_observations' || ':SELECT', 'public.' || 'recommendation_preview_batches' || ':SELECT', 'public.' || 'recommendation_run_campaigns' || ':SELECT', 'public.recommendation_runs:SELECT', 'public.recommendation_runs:UPDATE', 'public.recommendations:INSERT', 'public.recommendations:SELECT', 'public.sync_jobs:SELECT', 'public.sync_jobs:UPDATE', 'public.targets:SELECT']::text[]) expected(entry)
  where not exists (
    select 1
    from pg_catalog.pg_class relation
    join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
    cross join lateral aclexplode(relation.relacl) acl
    where namespace.nspname || '.' || relation.relname || ':' || acl.privilege_type
            = expected.entry
      and acl.grantee = 'openspell_recommendation_executor'::regrole
  )
  union all
  select 'wp196.executor_relation_acl_unexpected', '0', count(*)::text
  from pg_catalog.pg_class relation
  join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
  cross join lateral aclexplode(relation.relacl) acl
  where acl.grantee = 'openspell_recommendation_executor'::regrole
    and namespace.nspname || '.' || relation.relname || ':' || acl.privilege_type
        <> all(array['public.ad_groups:SELECT', 'public.ad_profiles:SELECT', 'public.apply_batches:SELECT', 'public.apply_rows:SELECT', 'public.audit_log:INSERT', 'public.audit_log_id_seq:SELECT', 'public.audit_log_id_seq:USAGE', 'public.bid_series_daily:SELECT', 'public.campaigns:SELECT', 'public.fact_profile_daily:SELECT', 'public.fact_sb_daily:SELECT', 'public.fact_sd_daily:SELECT', 'public.fact_sp_target_daily:SELECT', 'public.keywords:SELECT', 'public.product_ads:SELECT', 'public.rank_observations:SELECT', 'public.' || 'recommendation_observations' || ':SELECT', 'public.' || 'recommendation_preview_batches' || ':SELECT', 'public.' || 'recommendation_run_campaigns' || ':SELECT', 'public.recommendation_runs:SELECT', 'public.recommendation_runs:UPDATE', 'public.recommendations:INSERT', 'public.recommendations:SELECT', 'public.sync_jobs:SELECT', 'public.sync_jobs:UPDATE', 'public.targets:SELECT']::text[])
  union all
  select 'wp196.worker_execute_missing', '0', count(*)::text
  from unnest(array['claim_recommendation_jobs_fenced', 'defer_recommendation_job_fenced', 'fail_recommendation_run_fenced', 'finish_recommendation_job_fenced', 'get_recommendation_claim_authority', 'get_recommendation_cutover_evidence', 'get_recommendation_worker_authority', 'read_recommendation_inputs_fenced', 'resume_recommendation_jobs_fenced', 'start_recommendation_run_fenced', 'succeed_recommendation_run_fenced']::text[]) expected(name)
  where not exists (
    select 1 from pg_catalog.pg_proc routine
    cross join lateral aclexplode(routine.proacl) acl
    where routine.proname = expected.name
      and acl.grantee = 'openspell_recommendation_worker'::regrole
      and acl.privilege_type = 'EXECUTE'
  )
  union all
  select 'wp196.worker_execute_unexpected', '0', count(*)::text
  from pg_catalog.pg_proc routine
  cross join lateral aclexplode(routine.proacl) acl
  where acl.grantee = 'openspell_recommendation_worker'::regrole
    and acl.privilege_type = 'EXECUTE'
    and routine.proname <> all(array['claim_recommendation_jobs_fenced', 'defer_recommendation_job_fenced', 'fail_recommendation_run_fenced', 'finish_recommendation_job_fenced', 'get_recommendation_claim_authority', 'get_recommendation_cutover_evidence', 'get_recommendation_worker_authority', 'read_recommendation_inputs_fenced', 'resume_recommendation_jobs_fenced', 'start_recommendation_run_fenced', 'succeed_recommendation_run_fenced']::text[])
  union all
  select 'wp196.executor_owned_routine_missing', '0', count(*)::text
  from unnest(array['claim_recommendation_jobs_fenced', 'defer_recommendation_job_fenced', 'fail_recommendation_run_fenced', 'finish_recommendation_job_fenced', 'lock_recommendation_claimed_run', 'read_recommendation_inputs_fenced', 'resume_recommendation_jobs_fenced', 'start_recommendation_run_fenced', 'succeed_recommendation_run_fenced']::text[]) expected(name)
  where not exists (
    select 1 from pg_catalog.pg_proc routine
    where routine.proname = expected.name
      and routine.proowner = 'openspell_recommendation_executor'::regrole
  )
  union all
  select 'wp196.executor_owned_routine_unexpected', '0', count(*)::text
  from pg_catalog.pg_proc routine
  where routine.proowner = 'openspell_recommendation_executor'::regrole
    and routine.proname <> all(array['claim_recommendation_jobs_fenced', 'defer_recommendation_job_fenced', 'fail_recommendation_run_fenced', 'finish_recommendation_job_fenced', 'lock_recommendation_claimed_run', 'read_recommendation_inputs_fenced', 'resume_recommendation_jobs_fenced', 'start_recommendation_run_fenced', 'succeed_recommendation_run_fenced']::text[])
  union all
  select 'wp196.executor_policy_missing', '0', count(*)::text
  from unnest(array['ad_groups:recommendation_executor_select:SELECT', 'ad_profiles:recommendation_executor_select:SELECT', 'apply_batches:recommendation_executor_select:SELECT', 'apply_rows:recommendation_executor_select:SELECT', 'audit_log:recommendation_executor_insert:INSERT', 'bid_series_daily:recommendation_executor_select:SELECT', 'campaigns:recommendation_executor_select:SELECT', 'fact_profile_daily:recommendation_executor_select:SELECT', 'fact_sb_daily:recommendation_executor_select:SELECT', 'fact_sd_daily:recommendation_executor_select:SELECT', 'fact_sp_target_daily:recommendation_executor_select:SELECT', 'keywords:recommendation_executor_select:SELECT', 'product_ads:recommendation_executor_select:SELECT', 'rank_observations:recommendation_executor_select:SELECT', 'recommendation_observations:recommendation_executor_select:SELECT', 'recommendation_preview_batches:recommendation_executor_select:SELECT', 'recommendation_run_campaigns:recommendation_executor_select:SELECT', 'recommendation_runs:recommendation_executor_select:SELECT', 'recommendation_runs:recommendation_executor_update:UPDATE', 'recommendations:recommendation_executor_insert:INSERT', 'recommendations:recommendation_executor_select:SELECT', 'sync_jobs:recommendation_executor_select:SELECT', 'sync_jobs:recommendation_executor_update:UPDATE', 'targets:recommendation_executor_select:SELECT']::text[]) expected(entry)
  where not exists (
    select 1 from pg_catalog.pg_policy policy
    join pg_catalog.pg_class relation on relation.oid = policy.polrelid
    where relation.relname || ':' || policy.polname || ':' ||
          case policy.polcmd when 'r' then 'SELECT' when 'a' then 'INSERT'
                             when 'w' then 'UPDATE' when 'd' then 'DELETE'
                             else 'ALL' end = expected.entry
      and 'openspell_recommendation_executor'::regrole = any(policy.polroles)
  )
  union all
  select 'wp196.executor_policy_unexpected', '0', count(*)::text
  from pg_catalog.pg_policy policy
  join pg_catalog.pg_class relation on relation.oid = policy.polrelid
  where 'openspell_recommendation_executor'::regrole = any(policy.polroles)
    and relation.relname || ':' || policy.polname || ':' ||
        case policy.polcmd when 'r' then 'SELECT' when 'a' then 'INSERT'
                           when 'w' then 'UPDATE' when 'd' then 'DELETE'
                           else 'ALL' end
        <> all(array['ad_groups:recommendation_executor_select:SELECT', 'ad_profiles:recommendation_executor_select:SELECT', 'apply_batches:recommendation_executor_select:SELECT', 'apply_rows:recommendation_executor_select:SELECT', 'audit_log:recommendation_executor_insert:INSERT', 'bid_series_daily:recommendation_executor_select:SELECT', 'campaigns:recommendation_executor_select:SELECT', 'fact_profile_daily:recommendation_executor_select:SELECT', 'fact_sb_daily:recommendation_executor_select:SELECT', 'fact_sd_daily:recommendation_executor_select:SELECT', 'fact_sp_target_daily:recommendation_executor_select:SELECT', 'keywords:recommendation_executor_select:SELECT', 'product_ads:recommendation_executor_select:SELECT', 'rank_observations:recommendation_executor_select:SELECT', 'recommendation_observations:recommendation_executor_select:SELECT', 'recommendation_preview_batches:recommendation_executor_select:SELECT', 'recommendation_run_campaigns:recommendation_executor_select:SELECT', 'recommendation_runs:recommendation_executor_select:SELECT', 'recommendation_runs:recommendation_executor_update:UPDATE', 'recommendations:recommendation_executor_insert:INSERT', 'recommendations:recommendation_executor_select:SELECT', 'sync_jobs:recommendation_executor_select:SELECT', 'sync_jobs:recommendation_executor_update:UPDATE', 'targets:recommendation_executor_select:SELECT']::text[])
  union all
  select 'wp196.narrow_role_default_acls', '0', count(*)::text
  from pg_catalog.pg_default_acl default_acl
  ...
  union all
  select 'wp196.worker_execute_signature_missing', '0', count(*)::text
  from (values ('public', 'claim_recommendation_jobs_fenced', 'text, text, integer'), ('public', 'resume_recommendation_jobs_fenced', 'text, text'), ('public', 'finish_recommendation_job_fenced', 'uuid, text, uuid, text, sync_job_status, text, jsonb, interval'), ('public', 'defer_recommendation_job_fenced', 'uuid, text, uuid, text, interval'), ('public', 'get_recommendation_claim_authority', ''), ('public', 'get_recommendation_cutover_evidence', ''), ('public', 'get_recommendation_worker_authority', ''), ('public', 'start_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid'), ('public', 'fail_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, text'), ('public', 'succeed_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, jsonb'), ('public', 'read_recommendation_inputs_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, date, date')) expected(schema_name, routine_name, arguments)
  where not exists (
    select 1 from pg_catalog.pg_proc routine
    join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
    cross join lateral aclexplode(routine.proacl) acl
    where namespace.nspname = expected.schema_name
      ...
      and pg_catalog.oidvectortypes(routine.proargtypes) = expected.arguments
      and acl.grantee = 'openspell_recommendation_worker'::regrole
      and acl.privilege_type = 'EXECUTE'
  )
  union all
  select 'wp196.worker_execute_signature_unexpected', '0', count(*)::text
  from pg_catalog.pg_proc routine
  join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
  cross join lateral aclexplode(routine.proacl) acl
  where acl.grantee = 'openspell_recommendation_worker'::regrole
    and acl.privilege_type = 'EXECUTE'
    and not exists (
      select 1 from (values ('public', 'claim_recommendation_jobs_fenced', 'text, text, integer'), ('public', 'resume_recommendation_jobs_fenced', 'text, text'), ('public', 'finish_recommendation_job_fenced', 'uuid, text, uuid, text, sync_job_status, text, jsonb, interval'), ('public', 'defer_recommendation_job_fenced', 'uuid, text, uuid, text, interval'), ('public', 'get_recommendation_claim_authority', ''), ('public', 'get_recommendation_cutover_evidence', ''), ('public', 'get_recommendation_worker_authority', ''), ('public', 'start_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid'), ('public', 'fail_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, text'), ('public', 'succeed_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, jsonb'), ('public', 'read_recommendation_inputs_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, date, date'))
        expected(schema_name, routine_name, arguments)
      where namespace.nspname = expected.schema_name
        and routine.proname = expected.routine_name
        and pg_catalog.oidvectortypes(routine.proargtypes) = expected.arguments
    )
  union all
  select 'wp196.executor_owned_signature_missing', '0', count(*)::text
  from (values ('public', 'claim_recommendation_jobs_fenced', 'text, text, integer'), ('public', 'resume_recommendation_jobs_fenced', 'text, text'), ('public', 'finish_recommendation_job_fenced', 'uuid, text, uuid, text, sync_job_status, text, jsonb, interval'), ('public', 'defer_recommendation_job_fenced', 'uuid, text, uuid, text, interval'), ('public', 'start_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid'), ('public', 'fail_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, text'), ('public', 'succeed_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, jsonb'), ('public', 'read_recommendation_inputs_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, date, date'), ('app', 'lock_recommendation_claimed_run', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid')) expected(schema_name, routine_name, arguments)
  where not exists (
    select 1 from pg_catalog.pg_proc routine
    join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
    where namespace.nspname = expected.schema_name
  ...
  union all
  select 'wp196.executor_owned_signature_unexpected', '0', count(*)::text
  from pg_catalog.pg_proc routine
  join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
  where routine.proowner = 'openspell_recommendation_executor'::regrole
    and not exists (
  ...
  union all
  select 'wp196.executor_execute_signature_missing', '0', count(*)::text
  from (values ('app', 'assert_recommendation_worker', 'text'), ('app', 'recommendation_authority_snapshot', ''), ('app', 'recommendation_authority_claim_snapshot', ''), ('app', 'recommendation_job_scope_closes', 'uuid'), ('app', 'recommendation_job_scope_is_current', 'uuid'), ('app', 'lock_recommendation_claimed_run', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid'), ('public', 'claim_recommendation_jobs_fenced', 'text, text, integer'), ('public', 'resume_recommendation_jobs_fenced', 'text, text'), ('public', 'finish_recommendation_job_fenced', 'uuid, text, uuid, text, sync_job_status, text, jsonb, interval'), ('public', 'defer_recommendation_job_fenced', 'uuid, text, uuid, text, interval'), ('public', 'start_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid'), ('public', 'fail_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, text'), ('public', 'succeed_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, jsonb'), ('public', 'read_recommendation_inputs_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, date, date')) expected(schema_name, routine_name, arguments)
  where not exists (
    select 1 from pg_catalog.pg_proc routine
    join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
    cross join lateral aclexplode(routine.proacl) acl
  ...
  union all
  select 'wp196.executor_execute_signature_unexpected', '0', count(*)::text
  from pg_catalog.pg_proc routine
  join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
  cross join lateral aclexplode(routine.proacl) acl
  where acl.grantee = 'openspell_recommendation_executor'::regrole
    and acl.privilege_type = 'EXECUTE'
    and not exists (
      select 1 from (values ('app', 'assert_recommendation_worker', 'text'), ('app', 'recommendation_authority_snapshot', ''), ('app', 'recommendation_authority_claim_snapshot', ''), ('app', 'recommendation_job_scope_closes', 'uuid'), ('app', 'recommendation_job_scope_is_current', 'uuid'), ('app', 'lock_recommendation_claimed_run', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid'), ('public', 'claim_recommendation_jobs_fenced', 'text, text, integer'), ('public', 'resume_recommendation_jobs_fenced', 'text, text'), ('public', 'finish_recommendation_job_fenced', 'uuid, text, uuid, text, sync_job_status, text, jsonb, interval'), ('public', 'defer_recommendation_job_fenced', 'uuid, text, uuid, text, interval'), ('public', 'start_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid'), ('public', 'fail_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, text'), ('public', 'succeed_recommendation_run_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, jsonb'), ('public', 'read_recommendation_inputs_fenced', 'uuid, text, uuid, text, uuid, uuid, uuid, uuid, date, date'))
        expected(schema_name, routine_name, arguments)
      where namespace.nspname = expected.schema_name
        and routine.proname = expected.routine_name
        and pg_catalog.oidvectortypes(routine.proargtypes) = expected.arguments
  ...
  union all
  select 'wp196.executor_policy_shape_mismatch', '0', count(*)::text
  from pg_catalog.pg_policy policy
  join pg_catalog.pg_class relation on relation.oid = policy.polrelid
  where 'openspell_recommendation_executor'::regrole = any(policy.polroles)
    and (
      (relation.relname = 'sync_jobs'
       and (policy.polqual is null
         or pg_catalog.pg_get_expr(policy.polqual, policy.polrelid)
            not like '%recommendations.run%'
         or (policy.polcmd = 'w' and (
              policy.polwithcheck is null
              or pg_catalog.pg_get_expr(policy.polwithcheck, policy.polrelid)
                 not like '%recommendations.run%'
            ))))
      or
      (relation.relname <> 'sync_jobs' and policy.polcmd = 'r'
       and pg_catalog.pg_get_expr(policy.polqual, policy.polrelid) <> 'true')
      or
      (relation.relname <> 'sync_jobs' and policy.polcmd in ('a', 'w')
       and pg_catalog.pg_get_expr(policy.polwithcheck, policy.polrelid) <> 'true')
    )
  union all
  select 'wp192.service_wrapper_signature_missing', '0', count(*)::text
  from (values ('claim_sp_write_outbox', 'text, sp_write_outbox_kind[], integer, integer'), ('renew_sp_write_outbox_claim', 'uuid, bigint, uuid, integer'), ('defer_sp_write_outbox_claim', 'uuid, bigint, uuid, text'), ('complete_sp_write_outbox_claim', 'uuid, bigint, uuid'), ('acquire_sp_write_dispatch_lease_for_claim', 'uuid, bigint, uuid, sp_write_route_key, integer'), ('reserve_sp_write_provider_call_for_claim', 'uuid, bigint, uuid, uuid, uuid, uuid, uuid, text, text, text, text, text')) expected(routine_name, arguments)
  where not exists (
    select 1 from pg_catalog.pg_proc routine
    join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
    cross join lateral aclexplode(routine.proacl) acl
  ...
  union all
  select 'wp192.service_wrapper_signature_unexpected', '0', count(*)::text
  from pg_catalog.pg_proc routine
  join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
  cross join lateral aclexplode(routine.proacl) acl
  where namespace.nspname = 'app'
    ...
    and acl.privilege_type = 'EXECUTE'
    and not exists (
      select 1 from (values ('claim_sp_write_outbox', 'text, sp_write_outbox_kind[], integer, integer'), ('renew_sp_write_outbox_claim', 'uuid, bigint, uuid, integer'), ('defer_sp_write_outbox_claim', 'uuid, bigint, uuid, text'), ('complete_sp_write_outbox_claim', 'uuid, bigint, uuid'), ('acquire_sp_write_dispatch_lease_for_claim', 'uuid, bigint, uuid, sp_write_route_key, integer'), ('reserve_sp_write_provider_call_for_claim', 'uuid, bigint, uuid, uuid, uuid, uuid, uuid, text, text, text, text, text')) expected(routine_name, arguments)
      where routine.proname = expected.routine_name
        and pg_catalog.oidvectortypes(routine.proargtypes) = expected.arguments
    )
  union all
  select 'wp192.unfenced_service_execute', 'false:false',
         has_function_privilege(
           'service_role',
           'app.acquire_sp_write_dispatch_lease(uuid,uuid,uuid,sp_write_route_key,integer)',
           'EXECUTE'
         )::text || ':' ||
         has_function_privilege(
           'service_role',
  ...
  union all
  select 'wp194.service_signature_missing', '0', count(*)::text
  from (values ('claim_sync_jobs_fenced', 'text, integer, sync_job_type[]'), ('finish_sync_job_fenced', 'uuid, uuid, sync_job_status, text, jsonb, interval'), ('defer_sync_job_fenced', 'uuid, uuid, interval'), ('get_report_worker_claim_authority', ''), ('activate_report_worker_fenced_claims', '')) expected(routine_name, arguments)
  where not exists (
    select 1 from pg_catalog.pg_proc routine
    join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
    cross join lateral aclexplode(routine.proacl) acl
  ...
  union all
  select 'wp194.service_signature_unexpected', '0', count(*)::text
  from pg_catalog.pg_proc routine
  join pg_catalog.pg_namespace namespace on namespace.oid = routine.pronamespace
  cross join lateral aclexplode(routine.proacl) acl
  where namespace.nspname = 'public'
    and routine.proname = any(array[
      'claim_sync_jobs_fenced', 'finish_sync_job_fenced',
      'defer_sync_job_fenced', 'get_report_worker_claim_authority',
      'activate_report_worker_fenced_claims'
    ]::text[])
    and acl.grantee = 'service_role'::regrole
    and acl.privilege_type = 'EXECUTE'
    and not exists (
      select 1 from (values ('claim_sync_jobs_fenced', 'text, integer, sync_job_type[]'), ('finish_sync_job_fenced', 'uuid, uuid, sync_job_status, text, jsonb, interval'), ('defer_sync_job_fenced', 'uuid, uuid, interval'), ('get_report_worker_claim_authority', ''), ('activate_report_worker_fenced_claims', '')) expected(routine_name, arguments)
      where routine.proname = expected.routine_name
        and pg_catalog.oidvectortypes(routine.proargtypes) = expected.arguments
    )
  union all
  select 'wp194.report_authority_direct_grants', '0', count(*)::text
  from pg_catalog.pg_class relation
  join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
  cross join lateral aclexplode(relation.relacl) acl
  where namespace.nspname = 'app'
    and relation.relname = 'report_worker_claim_authority'
    and acl.grantee in (
      0, 'anon'::regrole, 'authenticated'::regrole, 'service_role'::regrole
    )
),
