...
describe('release verifier subprocess boundary', () => {
  ...
  it.each([
    ...
    try {
      const result = run({ PATH: `${fixture.bin}:${process.env['PATH'] ?? ''}`, OPENSPELL_RELEASE_CANDIDATE_URL: `https://${HOST}` });
      const output = `${result.stdout}${result.stderr}`;
...
function makeCurl(mode: 'healthy' | 'mismatch' | 'not-ready' | 'redirect' | 'fail') {
...
if [ "$n" -eq 1 ]; then
  ...
  printf 'HTTP/1.1 ${metadataStatus} Status\\r\\n\\r\\n{"url":"${metadataHost}","projectId":"project-synthetic","ownerId":"org-synthetic","readyState":"${ready}"}\\nOPENSPELL_STATUS:${metadataStatus}'
else
