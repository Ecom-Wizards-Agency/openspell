...
export async function loadQueryIntelligenceSource(
  ...
  const [facts, ppc, vocabulary, proposals, negativeExports, promotionRuns] = await Promise.all([
    readFacts(handle, input),
    readPpc(handle, input),
    readVocabulary(handle, input),
    listContextualNegativeProposals(handle, input),
    listContextualNegativeExports(handle, input),
    readPromotionEvidence(handle, input),
  ]);
  return { facts, ppc, vocabulary, proposals, negativeExports, promotionRuns };
}
