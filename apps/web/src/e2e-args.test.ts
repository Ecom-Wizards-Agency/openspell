...
describe('web E2E runner arguments', () => {
  ...
  it('rejects an unknown suite instead of silently running everything', () => {
    expect(() => parseE2EArgs(['unknown'])).toThrow(
      "Unknown suite 'unknown'. Expected one of: tags-goto, grid-performance, optimization-groups, profile-context, auth, auth-members, auth-oauth, auth-roles, route-acceptance, all.",
    );
  });
});
